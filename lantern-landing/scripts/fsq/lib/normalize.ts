...
export function isJapaneseName(name: string): boolean {
  // Unicode ranges for CJK Unified Ideographs, Hiragana, Katakana, and CJK Compatibility
  return /[　-鿿豈-﫿＀-￯]/.test(name);
}

/**
 * Returns the best English name for a place.
 * If the primary name is non-Latin (e.g. Japanese), look for an "en" entry
 * in nameVariants. Otherwise, use the primary name itself.
 */
export function detectEnglishName(
  primaryName: string,
  nameVariants: Record<string, string>,
): string {
  if (!isJapaneseName(primaryName)) return primaryName;
...
export function normalizeFsqPlace(
  ...
  return {
    ...
    license:            "CC-BY-4.0",
    name_local:
      isJapaneseName(place.namePrimary) && place.namePrimary !== title
        ? place.namePrimary
        : undefined,
    name_alts:
