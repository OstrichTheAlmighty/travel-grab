...
st.title("Where can I go next?")
st.caption("AI-native travel planning for personalized, premium trips.")
...
SEMI_FLEXIBLE_CATEGORIES = {"Subscriptions"}
...
def load_demo_transactions_if_needed():
    ...
    if transactions and any(str(tx.get("source") or "").lower() == "controlled_demo" for tx in transactions):
        return api_get_transactions(month_start.isoformat(), month_end.isoformat()).get("transactions", [])
...
def local_sample_transactions(as_of):
    may_2026 = datetime.date(2026, 5, 1)
    week_starts = [
    ...
    weekly_amounts = [
        ("normal", {"Restaurants / dining": -70, "Shopping": -55, "Entertainment": -40, "Coffee": -20, "Subscriptions": -25}),
        ("overspend", {"Restaurants / dining": -120, "Shopping": -110, "Entertainment": -50, "Coffee": -26, "Subscriptions": -25}),
        ("improved", {"Restaurants / dining": -45, "Shopping": -35, "Entertainment": -28, "Coffee": -12, "Subscriptions": -25}),
        ("good", {"Restaurants / dining": -30, "Shopping": -20, "Entertainment": -15, "Coffee": -8, "Subscriptions": -25}),
    ]
    fixed = {
        "Income": ("Employer Payroll", 750.0),
        "Bills": ("Rent / Bills", -500.0),
        "Groceries": ("Grocery Market", -90.0),
        "Transportation": ("Transit & Fuel", -35.0),
        "Savings": ("Savings transfer - Hawaii trip", 80.0),
    }
    variable_merchants = {
        "Restaurants / dining": "Restaurants / dining",
        "Shopping": "Shopping",
    ...
    txs = []
    for week_index, (scenario, variable) in enumerate(weekly_amounts, start=1):
        start = week_starts[week_index - 1]
        source = "controlled_demo"
        txs.extend(
            [
                _local_tx(start, fixed["Income"][0], fixed["Income"][1], "Income", source, scenario),
                _local_tx(start + datetime.timedelta(days=1), fixed["Bills"][0], fixed["Bills"][1], "Bills", source, scenario),
                _local_tx(start + datetime.timedelta(days=2), fixed["Groceries"][0], fixed["Groceries"][1], "Groceries", source, scenario),
                _local_tx(start + datetime.timedelta(days=3), fixed["Transportation"][0], fixed["Transportation"][1], "Transportation", source, scenario),
                _local_tx(start + datetime.timedelta(days=4), fixed["Savings"][0], fixed["Savings"][1], "Savings", source, scenario),
            ]
        )
        for offset, (category, amount) in enumerate(variable.items()):
            txs.append(
                _local_tx(
                    start + datetime.timedelta(days=min(5, offset)),
                    variable_merchants[category],
                    amount,
                    category,
                    source,
                    scenario,
...
def local_load_realistic_transaction_history(as_of):
    ...
    st.session_state.local_transactions = sample
    ...
    st.session_state.goal_input_name = "Hawaii trip"
    st.session_state.goal_input_cost = 1800.0
    st.session_state.goal_input_date = datetime.date(2026, 8, 18)
    st.session_state.pop("goal_plan", None)
    return {
        "status": "ok",
        "loaded": len(sample),
...
def local_simulate_week(scenario, as_of):
    ...
    tracked = {"Food", "Coffee", "Restaurants / dining", "Shopping", "Entertainment", "Subscriptions", "Other", "Other discretionary"}
    kept = []
    baseline_totals = {}
    deleted = 0
...
def savings_transfer_total_for_week(transactions, week_start, week_end):
    total = 0.0
    for tx in transactions:
        ...
        if category != SAVINGS_CATEGORY.lower() and "savings transfer" not in merchant:
            continue
        tx_date = parse_tx_date(tx)
...
def controlled_demo_as_of(transactions, selected_month, fallback):
    month_start_date, month_end_date = month_bounds(selected_month)
    dates = [
        parse_tx_date(tx)
        for tx in transactions
        if str(tx.get("source") or "").lower() == "controlled_demo"
        and month_start_date <= parse_tx_date(tx) <= month_end_date
    ]
    return max(dates) if dates else fallback


def selected_month_controlled_baseline(transactions, selected_month):
    """Use Week 1 of the controlled demo as the spending baseline when no prior-month data exists."""
    baseline = {}
    month_start_date, month_end_date = month_bounds(selected_month)
    for tx in transactions:
        ...
        if str(tx.get("source") or "").lower() != "controlled_demo":
            continue
        if str(tx.get("scenario") or "").lower() != "normal":
            continue
        tx_date = parse_tx_date(tx)
        if not (month_start_date <= tx_date <= month_end_date):
            continue
        category_name = intelligence_bucket(tx)
        if category_name not in BEHAVIOR_TRACKED_CATEGORIES:
            continue
        baseline[category_name] = baseline.get(category_name, 0.0) + abs(float(tx.get("amount", 0.0)))
    return baseline
...
def calculate_goal_analysis(
    ...
    as_of = controlled_demo_as_of(transactions, selected_month, as_of)
    ...
    if not baseline:
        baseline = selected_month_controlled_baseline(transactions, selected_month)
        baseline_weeks = 1.0
    ...
    savings_transfer_this_week = savings_transfer_total_for_week(
        transactions,
        current_week_start,
        min(as_of, current_week_end),
    )
    saved_toward_goal_this_week = max(0.0, behavior_improvement) + savings_transfer_this_week
    starting_remaining = max(0.0, float(goal_cost) - max(0.0, float(goal_progress)))
    ...
    gap_vs_target = saved_toward_goal_this_week - computed_weekly_target
    ...
    trajectory_status = trajectory_status_from_gap(gap_vs_target, computed_weekly_target)
    trajectory.update(
        {
            "detectedSavings": saved_toward_goal_this_week,
            "savedTowardGoalThisWeek": saved_toward_goal_this_week,
            "gapVsTarget": gap_vs_target,
            "behaviorGap": behavior_improvement - computed_weekly_target,
            "savingsGap": gap_vs_target,
            "remaining": remaining,
            "projectedDate": projected_date,
    ...
    return {
        ...
        "recommended_actions": recommended_actions,
        "number_of_simulation_transactions": simulation_count,
        "savingsTransferThisWeek": savings_transfer_this_week,
...
def build_goal_plan(goal_name, goal_cost, target_date, allow_protected):
    ...
    analysis_as_of = controlled_demo_as_of(all_transactions, month_start, as_of_date)
    ...
    transaction_intelligence = build_transaction_affordability_intelligence(
        all_transactions,
        analysis_as_of,
        remaining_amount,
        weekly_needed,
    )
    goal_analysis["transactionIntelligence"] = transaction_intelligence
    goal_analysis["realisticWeeklySavingsPotential"] = transaction_intelligence["realistic_weekly_savings_potential"]
    goal_analysis["confidenceScore"] = transaction_intelligence["confidence_score"]
    goal_analysis["confidenceLabel"] = transaction_intelligence["confidence_label"]
    goal_analysis["scenarioOutcomes"] = transaction_intelligence["scenario_outcomes"]
    improved_categories = {
        row["Category"]
    ...
    plan = {
        ...
        "dining_streak": dining_streak(all_transactions, analysis_as_of),
        "momentum": savings_momentum(all_transactions, analysis_as_of),
...
def travel_source_quality(idea):
    has_amadeus_credentials = bool(get_secret_value("AMADEUS_CLIENT_ID") and get_secret_value("AMADEUS_CLIENT_SECRET"))
    has_api_pricing = bool(idea.get("api_pricing_source") or idea.get("amadeus_pricing"))
    ...
    if has_amadeus_credentials and has_api_pricing and source_type in {"verified_booking_page", "verified_product_page"}:
        return {
            "label": "Price source",
...
def first_sentence(text, fallback=""):
    cleaned = str(text or fallback or "").strip()
    if not cleaned:
        return ""
...
def trip_budget_weights(travel_style, idea):
    style = str(travel_style or "balanced").lower()
    if style == "budget":
        weights = {
            "Flights": 0.32,
    ...
    if "scuba" in trip_text_bundle(idea) or "dive" in trip_text_bundle(idea):
...
def category_from_trip_budget_item(raw_item):
    lowered = str(raw_item or "").strip().lower()
    return next((label for token, label in TRIP_BUDGET_ALIASES.items() if token in lowered), "")


def build_canonical_trip_budget(idea, starting_city, trip_length, travel_style, target_budget=0.0):
    """Single source of truth for trip pricing.

    The displayed total, affordability goal cost, and breakdown all read from this object.
    Category amounts always sum to the total.
    """
    ...
    categories = {
        category: {
            ...
            "confidence": "fallback",
            "origin": "fallback",
            "note": default_cost_note(category, starting_city, trip_length),
        }
        for category in TRIP_BUDGET_CATEGORIES
    }
    if isinstance(raw_breakdown, list) and raw_breakdown:
        for row in raw_breakdown:
            if not isinstance(row, dict):
                continue
            category = category_from_trip_budget_item(row.get("item"))
            if not category:
                continue
            amount = parse_goal_card_cost(row.get("estimate") or row.get("cost") or row.get("amount"))
            if amount is None:
                continue
            categories[category]["amount"] += float(amount)
            categories[category]["confidence"] = "estimated"
            categories[category]["origin"] = "AI-generated estimate"
            note = str(row.get("notes") or row.get("note") or "").strip()
            if note:
                categories[category]["note"] = note
    subtotal = round(sum(row["amount"] for row in categories.values()), 2)
    if subtotal <= 0:
        base_total = parse_goal_card_cost(idea.get("estimated_cost")) or parse_goal_card_cost(target_budget) or 0.0
        weights = trip_budget_weights(travel_style, idea)
        running = 0.0
        for category in TRIP_BUDGET_CATEGORIES:
            if category == "Buffer":
                continue
            amount = round(float(base_total) * weights.get(category, 0.0), 2)
            categories[category]["amount"] = amount
            categories[category]["confidence"] = "heuristic"
            categories[category]["origin"] = "user-personalized assumption" if target_budget else "AI-generated estimate"
            running += amount
        categories["Buffer"]["amount"] = round(float(base_total) - running, 2)
        categories["Buffer"]["confidence"] = "heuristic"
        categories["Buffer"]["origin"] = "user-personalized assumption" if target_budget else "AI-generated estimate"
    if source_quality["primary_pricing"]:
        for row in categories.values():
            row["confidence"] = "live"
            row["origin"] = "live API data"
    total = round(sum(row["amount"] for row in categories.values()), 2)
    return {
        "categories": categories,
        "total": total,
        "confidence": "live" if source_quality["primary_pricing"] else ("estimated" if isinstance(raw_breakdown, list) and raw_breakdown else "heuristic"),
        "pricing_basis": "live API data" if source_quality["primary_pricing"] else ("AI-generated estimates" if isinstance(raw_breakdown, list) and raw_breakdown else "user-personalized assumptions"),
    }


def ensure_trip_budget(idea, starting_city, trip_length, travel_style, target_budget=0.0):
    budget = idea.get("trip_budget")
    if isinstance(budget, dict) and isinstance(budget.get("categories"), dict):
        return budget
    budget = build_canonical_trip_budget(idea, starting_city, trip_length, travel_style, target_budget)
    idea["trip_budget"] = budget
    idea["estimated_cost"] = budget["total"]
    return budget


def trip_budget_rows(trip_budget):
    categories = trip_budget.get("categories", {})
    rows = []
    for category in TRIP_BUDGET_CATEGORIES:
        ...
        rows.append(
            {
                "Item": category,
                "Estimate": money(row.get("amount", 0.0)),
                "Notes": row.get("note", ""),
                "Confidence": row.get("confidence", "fallback"),
                "Source quality": row.get("origin", "fallback"),
            }
        )
    rows.append(
        {
            "Item": "Total",
            "Estimate": money(trip_budget.get("total", 0.0)),
            "Notes": "Sum of canonical trip budget categories.",
            "Confidence": trip_budget.get("confidence", "fallback"),
            "Source quality": trip_budget.get("pricing_basis", "fallback"),
        }
    )
    return rows


def build_trip_cost_breakdown(idea, starting_city, trip_length, travel_style, target_budget=0.0):
    budget = ensure_trip_budget(idea, starting_city, trip_length, travel_style, target_budget)
    return trip_budget_rows(budget)
...
def build_trip_source_links(idea, starting_city, target_date):
    ...
    base_query = f"{destination} {target_label}".strip()
    links = [
        {
            "label": "Flight search",
            "url": "https://www.google.com/travel/flights?q=" + quote_plus(f"flights from {starting_city or 'my city'} to {destination} {target_label}"),
            "note": "Flights — verify live fares",
        },
        {
            "label": "Hotel search",
            "url": "https://www.booking.com/searchresults.html?ss=" + quote_plus(destination),
            "note": "Hotels — verify nightly rates",
        },
        {
            "label": "Activities search",
            "url": "https://www.viator.com/searchResults/all?" + "text=" + quote_plus(f"{destination} activities food tour"),
            "note": "Activities — verify tour prices",
        },
        {
            "label": "Destination research",
            "url": "https://www.google.com/maps/search/" + quote_plus(f"{destination} food neighborhoods things to do"),
            "note": "Food/places — research neighborhoods",
        },
    ]
    if source_url and not domain_matches(source_domain(source_url), BLOCKED_SOURCE_DOMAINS):
        links.append(
            {
                "label": "Attached source",
                "url": source_url,
                "note": source_title or "Source link — verify details",
            }
        )
    return links


def render_source_link_rows(links, labels=None):
    labels = labels or {}
    for link in links:
        ...
        if label in labels:
            ...
            st.markdown(f"**{label}**")
        st.caption(link.get("note", "Search link — verify live prices"))
        st.link_button("Open search link", link.get("url", "#"), width="stretch")


def monthly_affordability_status(required_monthly):
    try:
        active_budget = api_get_budget_active()
    except Exception:
        active_budget = st.session_state.get("local_budget", {})
    summary = budget_summary_defaults(active_budget)
    income = float(summary.get("income", 0.0) or 0.0)
    planned_spending = float(summary.get("essential_spending", 0.0) or 0.0) + float(
        summary.get("flexible_spending_limit", 0.0) or 0.0
    )
    allocations = (active_budget or {}).get("allocations", {}) or {}
    allocation_total = sum(float(value or 0.0) for value in allocations.values())
    if allocation_total > 0:
        planned_spending = allocation_total
    available = income - planned_spending
    gap = available - float(required_monthly or 0.0)
    if income <= 0:
        status = "Add income in Budget Planner to check affordability."
    elif gap >= 0:
        status = f"On track with {money(gap)}/month extra room."
    else:
        status = f"Short by {money(abs(gap))}/month under the current budget."
    return {
        "income": income,
...
def render_vertical_cost_breakdown(trip_budget):
    categories = trip_budget.get("categories", {}) or {}
    for category in TRIP_BUDGET_CATEGORIES:
        row = categories.get(category, {}) or {}
        st.markdown(
            f"**{category}:** {money(row.get('amount', 0.0))} — "
            f"{row.get('note', default_cost_note(category, '', ''))}"
        )
        st.caption(f"{row.get('confidence', 'heuristic').title()} · {row.get('origin', 'planning estimate')}")
    st.markdown(f"**Total:** {money(trip_budget.get('total', 0.0))}")
    st.caption("Total always equals the sum of the component costs above.")


def destination_hero_image(destination, idea):
    text = trip_text_bundle({"title": idea.get("title", ""), "destination": destination, "description": idea.get("description", "")})
    image_map = [
        (("hawaii", "maui", "honolulu", "kauai"), "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1800&q=80"),
        (("tokyo", "japan", "kyoto", "osaka"), "https://images.unsplash.com/photo-1503899036084-c55cdd92da26?auto=format&fit=crop&w=1800&q=80"),
        (("seoul", "korea"), "https://images.unsplash.com/photo-1538485399081-7191377e8241?auto=format&fit=crop&w=1800&q=80"),
        (("paris", "france"), "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?auto=format&fit=crop&w=1800&q=80"),
        (("scuba", "dive", "beach", "island", "bali", "mexico"), "https://images.unsplash.com/photo-1500375592092-40eb2168fd21?auto=format&fit=crop&w=1800&q=80"),
        (("new york", "nyc"), "https://images.unsplash.com/photo-1485871981521-5b1fd3805eee?auto=format&fit=crop&w=1800&q=80"),
    ]
...
def render_overview_hero(idea, destination, starting_city, trip_length, travel_style, target_date, total, monthly, confidence):
    style_label = {"budget": "Budget", "balanced": "Standard", "premium": "Luxury"}.get(
        str(travel_style or "balanced").lower(),
        str(travel_style or "Standard").title(),
    )
    hero_image = destination_hero_image(destination, idea)
    title = html.escape(str(idea.get("title") or destination or "Your Byable trip").strip())
    safe_destination = html.escape(str(destination or "Your trip").strip())
    safe_starting_city = html.escape(str(starting_city or "Your city").strip())
    safe_trip_length = html.escape(str(trip_length or "Flexible length").strip())
    safe_style_label = html.escape(str(style_label).strip())
    safe_confidence = html.escape(str(confidence).strip())
    subtitle = html.escape(first_sentence(
        idea.get("description") or idea.get("budget_angle") or "A personalized travel plan built around your timing, style, and estimated trip cost."
    ))
    html = f"""
    <style>
      .byable-hero {{
        ...
        overflow: hidden;
        background:
          linear-gradient(90deg, rgba(4,8,16,.94) 0%, rgba(4,8,16,.68) 42%, rgba(4,8,16,.22) 100%),
      ...
      .byable-soft-panel strong {{
        color: rgba(255,255,255,.96);
      }}
    </style>
    <div class="byable-hero">
      <div class="byable-hero-content">
        <div class="byable-kicker">Byable travel concierge</div>
        <h1>{title}</h1>
        <p>{subtitle}</p>
        <div class="byable-chip-row">
          <div class="byable-chip">Destination: {safe_destination}</div>
          <div class="byable-chip">From: {safe_starting_city}</div>
          <div class="byable-chip">{safe_trip_length}</div>
          <div class="byable-chip">{safe_style_label}</div>
          <div class="byable-chip">{target_date.strftime('%B %Y')}</div>
          <div class="byable-chip">{money(total)} estimate</div>
          <div class="byable-chip">{money(monthly)}/mo pace</div>
          <div class="byable-chip">{safe_confidence}</div>
        </div>
      </div>
    </div>
    """
...
def render_overview_soft_panel(title, body, caption=""):
    safe_title = html.escape(str(title or ""))
    safe_body = html.escape(str(body or ""))
    safe_caption = html.escape(str(caption or ""))
    st.markdown(
        f"""
        <div class="byable-soft-panel">
          <strong>{safe_title}</strong><br/>
          <span>{safe_body}</span>
          {f'<br/><small>{safe_caption}</small>' if safe_caption else ''}
        </div>
        """,
...
def send_trip_to_affordability_planner(idea, total, target_date):
    st.session_state.goal_input_name = idea.get("title", "Trip")
    st.session_state.goal_input_cost = float(total or 0.0)
    st.session_state.goal_input_date = target_date
...
def render_trip_workspace_tabs(idea, starting_city, trip_length, travel_style, target_date, source_quality):
    destination = str(idea.get("destination") or idea.get("title") or "Trip").strip()
    trip_budget = ensure_trip_budget(idea, starting_city, trip_length, travel_style)
    total = float(trip_budget.get("total", 0.0) or 0.0)
    monthly = calculate_monthly_savings_from_cost(total, target_date.strftime("%B %Y"))
    confidence = source_quality["confidence"] if source_quality else str(idea.get("confidence") or "Planning estimate")
    if not source_quality.get("primary_pricing", False):
        confidence = "Planning estimate"
    links = build_trip_source_links(idea, starting_city, target_date)
    flight_links = [link for link in links if "Flight" in link.get("label", "")]
    hotel_links = [link for link in links if "Hotel" in link.get("label", "")]
    activity_links = [link for link in links if "Activities" in link.get("label", "")]
    research_links = [link for link in links if "Destination" in link.get("label", "") or "Attached" in link.get("label", "")]

    tab_overview, tab_itinerary, tab_flights, tab_activities, tab_budget, tab_ai = st.tabs(
        ["Overview", "Itinerary", "Flights & Stay", "Activities", "Budget", "AI Picks"]
    )

    with tab_overview:
        render_overview_hero(idea, destination, starting_city, trip_length, travel_style, target_date, total, monthly, confidence)

        style_options = ["Budget", "Standard", "Luxury"]
        style_to_value = {"Budget": "budget", "Standard": "balanced", "Luxury": "premium"}
        value_to_style = {value: label for label, value in style_to_value.items()}
        current_style_label = value_to_style.get(str(travel_style or "balanced").lower(), "Standard")
        selected_style = st.radio(
            "Travel style",
            style_options,
            index=style_options.index(current_style_label),
            horizontal=True,
            key="overview_travel_style_selector",
            help="This is a planning preference today. Flights and hotels are prepared for future Amadeus API pricing.",
        )
        if style_to_value[selected_style] != str(st.session_state.get("travel_style", "balanced")).lower():
            st.caption("Regenerate the trip to apply this travel style to live recommendations.")

        st.markdown('<div class="byable-section-title">Trip summary</div>', unsafe_allow_html=True)
        render_overview_soft_panel(
            "Why this trip fits",
            first_sentence(idea.get("description") or idea.get("budget_angle") or f"{destination} matches your trip idea and target month."),
        )
        render_overview_soft_panel(
            "Estimated trip pricing",
            f"{money(total)} total planning estimate. Save about {money(monthly)} per month to be ready by {target_date.strftime('%B %Y')}.",
            "Prepared for live flight and hotel pricing once Amadeus Self-Service APIs are connected.",
        )

        strategy = default_flight_strategy(idea, starting_city, target_date, travel_style)
        hotel = default_hotel_strategy(idea, travel_style)
        highlights = default_food_experience_highlights(idea)
        activity_items = compact_text_list(highlights.get("famous_experiences"), limit=3)
        if not activity_items:
            activity_items = ["Anchor experience", "Neighborhood exploration", "One local food or culture stop"]
        picks = compact_text_list(idea.get("ways_to_afford_it"), limit=3) or [
            "Book flights and stay only after checking live fares.",
            "Choose one premium anchor activity and keep the rest flexible.",
            "Use the Budget tab to sanity-check the estimate before committing.",
        ]

        st.markdown('<div class="byable-section-title">Plan at a glance</div>', unsafe_allow_html=True)
        render_overview_soft_panel("Flight summary", strategy["routing_approach"], "Placeholder summary — verify live fares.")
        render_overview_soft_panel("Hotel summary", hotel["best_value_areas"], "Placeholder summary — verify live nightly rates.")
        render_overview_soft_panel("Activity highlights", " · ".join(activity_items))
        render_overview_soft_panel("AI optimization suggestions", " ".join(f"{idx}. {pick}" for idx, pick in enumerate(picks, start=1)))

        if not source_quality.get("primary_pricing", False):
            st.info("Planning estimate based on typical trip costs. Flights and hotels are search-link based until live travel APIs are added.")

        cta_cols = st.columns([1, 1])
        if cta_cols[0].button("Use this trip", key="overview_use_this_trip", type="primary", width="stretch"):
            try:
                send_trip_to_affordability_planner(idea, total, target_date)
                st.success("Trip saved into the planner.")
            except Exception as e:
                st.error("Could not send this trip into the planner.")
                if DEV_MODE:
                    st.code(str(e))
        if cta_cols[1].button("Customize trip", key="overview_customize_trip", width="stretch"):
            st.info("Adjust the starting city, destination idea, timing, budget, or style above, then generate a refreshed trip.")

    with tab_itinerary:
        st.markdown("### Clean vertical itinerary")
        for day in build_day_by_day_itinerary(idea, trip_length):
            with st.container(border=True):
                ...
                st.markdown(f"Dinner/night: {day['evening']}")
                st.markdown(f"Estimated daily spend: {money(day['estimated_daily_cost'])}")
    ...
    with tab_flights:
        ...
        st.markdown("### Flights")
        with st.container(border=True):
            st.markdown(f"**Best airport strategy:** {strategy['airport_strategy']}")
            st.markdown(f"**Cheapest routing approach:** {strategy['routing_approach']}")
            st.markdown(f"**Cheapest timing:** {strategy['best_timing']}")
            st.markdown(f"**Booking note:** {strategy['departure_tip']}")
            st.caption("Placeholder strategy — verify live fares before booking.")
            render_source_link_rows(flight_links)
        st.markdown("### Stay")
        with st.container(border=True):
            st.markdown(f"**Recommended neighborhoods:** {hotel['recommended_neighborhoods']}")
            st.markdown(f"**Best value areas:** {hotel['best_value_areas']}")
            for option in hotel["options"][:3]:
                st.markdown(f"**{option['tier']} — {option['nightly_cost']}/night:** {option['why']}")
            st.caption("Placeholder lodging strategy — verify live nightly rates before booking.")
            render_source_link_rows(hotel_links)

    with tab_activities:
        st.markdown("### Activities and experiences")
        highlights = default_food_experience_highlights(idea)
        sections = [
        ...
        for label, values in sections:
            items = compact_text_list(values, limit=4)
            if not items:
                continue
            with st.container(border=True):
                st.markdown(f"**{label}**")
                for item in items:
                    st.markdown(f"- {item}")
        st.markdown("### Activity search links")
        st.caption("Search links — verify live prices.")
        render_source_link_rows(activity_links + research_links)

    with tab_budget:
        st.markdown("### Trip budget")
        st.caption("This tab uses the selected trip budget object as the source of truth.")
        st.markdown(f"**Total trip cost:** {money(total)}")
        st.markdown(f"**Monthly savings needed:** {money(monthly)}")
        render_vertical_cost_breakdown(trip_budget)
        affordability = monthly_affordability_status(monthly)
        st.markdown("### Affordability status")
        st.markdown(f"**Monthly income:** {money(affordability['income'])}")
        st.markdown(f"**Planned future spending:** {money(affordability['planned_spending'])}")
        st.markdown(f"**Available for goals:** {money(affordability['available'])}")
        st.markdown(f"**Status:** {affordability['status']}")

    with tab_ai:
        st.markdown("### AI Picks")
        st.caption("Recommendation ideas live here so the main itinerary stays focused.")
        picks = compact_text_list(idea.get("ways_to_afford_it"), limit=5)
        if not picks:
            picks = [
                "Book the highest-cost component first after comparing live prices.",
                "Use flexible dining, shopping, entertainment, coffee, and subscriptions as tradeoff categories.",
                "Keep protected expenses out of the travel plan.",
            ]
        for idx, pick in enumerate(picks, start=1):
            with st.container(border=True):
                st.markdown(f"**Pick {idx}**")
                st.markdown(pick)
...
def normalize_ai_goal_card(card, target_month, location="", budget=0.0, enrich_live=False):
    ...
    normalized = {
        ...
        "confidence": str(raw_card.get("confidence") or "Planning estimate").strip(),
    ...
    if enrich_live:
        ...
        normalized["source_debug"] = {"query": search_query, "rejected_reason": "live enrichment skipped for faster initial generation"}
    return normalized
...
def render_goal_discovery():
    ...
    if not selected_goals and isinstance(ai_result, dict) and len(ai_result.get("valid_cards", [])) >= 1:
        selected_goals = ai_result["valid_cards"][:1]
    ...
    if isinstance(refreshed_goals, list) and len(refreshed_goals) >= 1:
        selected_goals = refreshed_goals[:1]
    ...
    if action_cols[0].button("Generate travel ideas", type="primary"):
        ...
        Generate exactly one excellent, budget-aware travel plan for Byable.
        ...
        title, destination, category, estimated_cost, monthly_savings, target_month,
        budget_angle, description, confidence, ways_to_afford_it, search_query.

        Rules:
        - category should be "Travel".
        - budget_angle should be one concise sentence explaining the cheapest or most realistic way to make the trip work.
        - confidence must be "Planning estimate" unless source-backed API pricing is explicitly provided.
        ...
        - Keep each field short. Do not generate a full itinerary yet.
        ...
        try:
            ...
            cards = [
                normalize_ai_goal_card(card, target_month, location=location, budget=target_budget, enrich_live=False)
    ...
    if (
        ...
        selected_goals = hard_rule_cards[:1]

    if active_goal_source:
        action_cols[2].caption(f"Source: {display_source_label(active_goal_source)}. Search links are included in the itinerary.")
    else:
        action_cols[2].caption("Search links are included in the itinerary.")

    if not selected_goals:
        if not active_goal_source:
            st.info("Click Generate travel ideas to build one personalized itinerary.")
            return
        st.info("No matching trip templates yet. Try a broader destination idea or travel vibe.")
        return

    best_trip = selected_goals[0]
    idea = dict(best_trip)
    if idea.get("generated_by") != "AI" and not valid_catalog_source(idea):
        st.info("No valid travel plan source is available yet. Try generating a new plan.")
        return
    trip_budget = ensure_trip_budget(idea, starting_city, trip_length, travel_style, target_budget)
    if st.session_state.get("goal_cards_key") == discovery_key:
        st.session_state["goal_cards"] = [idea]
    canonical_total = float(trip_budget.get("total", 0.0) or 0.0)
    monthly = idea.get("monthly_savings")
    if monthly in {None, ""}:
        monthly = calculate_monthly_savings_from_cost(canonical_total, idea.get("target_month") or target_month)
    else:
        monthly = calculate_monthly_savings_from_cost(canonical_total, idea.get("target_month") or target_month)
    target_date = parse_discovery_target_date(idea.get("target_month") or target_month)
    source_quality = travel_source_quality(idea)
    confidence = str(idea.get("confidence") or "Planning estimate").strip()
    if confidence.lower() == "verified estimate" and not source_quality["primary_pricing"]:
        confidence = "Planning estimate"

    with st.container(border=True):
        st.subheader(idea.get("title", "Recommended trip"))
        destination = str(idea.get("destination") or "").strip()
        if destination:
            st.markdown(f"**Destination:** {destination}")
        st.caption(first_sentence(idea.get("description") or idea.get("budget_angle") or "A budget-aware travel plan built around your timeline."))
        st.markdown(f"**Estimated total:** {money(canonical_total)}")
        st.markdown(f"**Monthly savings needed:** {money(monthly)}")
        st.markdown(f"**Confidence:** {confidence}")
        if idea.get("budget_angle"):
            st.markdown(f"**Budget angle:** {first_sentence(idea['budget_angle'])}")
        if not source_quality["primary_pricing"]:
            st.info("Planning estimate based on typical trip costs. Verify live prices with the links below.")
        if DEV_MODE and idea.get("source_debug"):
            with st.expander("Source debug", expanded=False):
                st.json(idea["source_debug"])
        if st.button("Use this trip", key=f"use_recommended_trip_{discovery_key}", type="primary", width="stretch"):
            try:
                send_trip_to_affordability_planner(idea, canonical_total, target_date)
            except Exception as e:
                st.error("Could not send this trip into the planner.")
                if DEV_MODE:
                    st.code(str(e))
...
def week_starts_for_window(as_of, weeks_back=8):
    current_week_start, _ = week_bounds(as_of)
    return [current_week_start - datetime.timedelta(days=7 * offset) for offset in reversed(range(weeks_back))]


def discretionary_weekly_by_category(transactions, as_of, weeks_back=8):
    weeks = week_starts_for_window(as_of, weeks_back)
    first_week = weeks[0] if weeks else week_bounds(as_of)[0]
    week_set = set(weeks)
    by_category = {}
    for tx in transactions:
        amount = float(tx.get("amount", 0.0) or 0.0)
        ...
        if category_flexibility(category) != "flexible" and category not in BEHAVIOR_TRACKED_CATEGORIES:
            continue
        tx_date = parse_tx_date(tx)
        week_start, _ = week_bounds(tx_date)
        if week_start < first_week or week_start not in week_set:
            continue
        by_category.setdefault(category, {week: 0.0 for week in weeks})
        by_category[category][week_start] += abs(amount)
    return by_category, weeks
...
def projected_completion_from_weekly(remaining_amount, weekly_savings):
    weekly_savings = float(weekly_savings or 0.0)
    if weekly_savings <= 0:
        return None
    weeks_needed = float(remaining_amount or 0.0) / weekly_savings
    return today + datetime.timedelta(days=round(weeks_needed * 7))


def recent_income_consistency(transactions, as_of, months_back=4):
    month_keys = []
    ...
    for tx in transactions:
        ...
        merchant = str(tx.get("merchant", "")).lower()
        if any(word in merchant for word in ("transfer", "credit card", "payment", "savings")):
            continue
        if category and not any(word in category for word in ("income", "payroll", "salary")):
            continue
        tx_date = parse_tx_date(tx)
    ...
    avg = mean_value(values) if values else 0.0
    volatility = stddev_value(values) / max(avg, 1.0) if values else 1.0
    score = max(0.0, min(1.0, 1.0 - volatility))
    return {
        "monthly_income_values": values,
        "average_income": avg,
...
def build_transaction_affordability_intelligence(transactions, as_of, remaining_amount, required_weekly_savings):
    weekly_by_category, weeks = discretionary_weekly_by_category(transactions, as_of, weeks_back=8)
    current_week = week_bounds(as_of)[0]
    category_rows = []
    total_baseline = 0.0
    total_current = 0.0
    total_potential = 0.0
    all_week_totals = {week: 0.0 for week in weeks}
    for category, week_values in weekly_by_category.items():
        values = [float(week_values.get(week, 0.0)) for week in weeks]
        baseline_values = [value for value in values[:-1] if value > 0] or values[:-1] or values
        baseline = mean_value(baseline_values)
        current = float(week_values.get(current_week, values[-1] if values else 0.0))
        volatility = stddev_value(baseline_values) / max(baseline, 1.0)
        cut_rate = category_cut_rate(category)
        realistic_potential = max(0.0, baseline * min(cut_rate, 0.40))
        total_baseline += baseline
        total_current += current
        total_potential += realistic_potential
        for week, value in zip(weeks, values):
            all_week_totals[week] += value
        category_rows.append(
            {
                "Category": category,
                "Average weekly spend": baseline,
                "Current week spend": current,
                "Realistic weekly savings": realistic_potential,
                "Volatility": volatility,
                "Why it matters": f"{category} is discretionary and has a realistic trim rate of {round(min(cut_rate, 0.40) * 100)}%.",
            }
        )
    category_rows = sorted(category_rows, key=lambda row: row["Realistic weekly savings"], reverse=True)
    week_total_values = list(all_week_totals.values())
    spending_avg = mean_value(week_total_values)
    spending_volatility = stddev_value(week_total_values) / max(spending_avg, 1.0) if week_total_values else 1.0
    savings_weeks = sum(1 for value in week_total_values if spending_avg > 0 and value <= spending_avg * 0.90)
    savings_history_score = savings_weeks / max(1, len(week_total_values))
    income = recent_income_consistency(transactions, as_of)
    volatility_score = max(0.0, min(1.0, 1.0 - spending_volatility))
    confidence_score = round((income["score"] * 0.40 + volatility_score * 0.35 + savings_history_score * 0.25) * 100)
    ...
    scenarios = []
    scenario_inputs = [
        ("Good week", total_potential, "Uses the realistic weekly savings potential from discretionary categories."),
        ("Average week", max(0.0, total_potential * 0.55), "Assumes the user captures about half of realistic discretionary savings."),
        ("Overspend week", max(0.0, total_potential * 0.15), "Assumes most discretionary savings are missed this week."),
    ]
    for label, weekly_savings, explanation in scenario_inputs:
        projected = projected_completion_from_weekly(remaining_amount, weekly_savings)
        scenarios.append(
            {
                "Scenario": label,
                "Weekly savings": weekly_savings,
                "Projected completion": projected,
                "Required pace gap": weekly_savings - float(required_weekly_savings or 0.0),
                "Explanation": explanation,
            }
        )
    return {
        "discretionary_categories": category_rows,
        "realistic_weekly_savings_potential": total_potential,
        "baseline_weekly_discretionary": total_baseline,
        "current_week_discretionary": total_current,
        "spending_volatility": spending_volatility,
        "income_consistency": income,
        "savings_history_score": savings_history_score,
...
def render_transaction_intelligence(plan):
    intelligence = plan.get("transaction_intelligence") or plan.get("goalAnalysis", {}).get("transactionIntelligence", {})
    if not intelligence:
        return
    with st.container(border=True):
        st.subheader("Transaction-based affordability")
        st.caption("Deterministic analysis from transaction history. No AI-generated coaching is used here.")
        st.markdown(f"**Realistic weekly savings potential:** {money(intelligence.get('realistic_weekly_savings_potential', 0.0))}")
        st.markdown(
            f"**Planning confidence:** {intelligence.get('confidence_label', 'Low')} "
            f"({intelligence.get('confidence_score', 0)}/100)"
        )
        income = intelligence.get("income_consistency", {}) or {}
        st.caption(
            "Confidence uses income consistency, discretionary-spending volatility, and recent savings history. "
            f"Income consistency score: {round(float(income.get('score', 0.0)) * 100)}%. "
            f"Spending volatility: {round(float(intelligence.get('spending_volatility', 0.0)) * 100)}%."
        )

        category_rows = intelligence.get("discretionary_categories", [])[:5]
        if category_rows:
            st.markdown("**Discretionary categories detected**")
            for row in category_rows:
                st.markdown(
                    f"**{row['Category']}**: {money(row['Realistic weekly savings'])}/week potential "
                    f"from {money(row['Average weekly spend'])}/week normal spend."
                )
                st.caption(row["Why it matters"])

        st.markdown("**Scenario outcomes**")
        for row in intelligence.get("scenario_outcomes", []):
            projected = projected_date_label(row.get("Projected completion"))
            gap = float(row.get("Required pace gap", 0.0))
            gap_text = f"{money(gap)}/week ahead" if gap >= 0 else f"{money(abs(gap))}/week short"
            st.markdown(
                f"**{row['Scenario']}**: {money(row['Weekly savings'])}/week saved, "
                f"projected completion {projected}, {gap_text}."
            )
            st.caption(row.get("Explanation", ""))


def controlled_demo_week_label(tx):
    scenario = str(tx.get("scenario") or "").strip().lower()
    scenario_order = {"normal": 1, "overspend": 2, "improved": 3, "good": 4}
    if scenario in scenario_order:
        return f"Week {scenario_order[scenario]} ({scenario})"
    tx_date = parse_tx_date(tx)
    first_week = datetime.date(2026, 5, 4)
    week_number = max(1, ((tx_date - first_week).days // 7) + 1)
    return f"Week {week_number}"


def source_of_truth_transactions():
    transactions = api_get_all_transactions()
    controlled = [
        tx
        for tx in transactions
    ...
    return controlled or transactions


def render_affordability_source_of_truth(plan):
    transactions = sorted(source_of_truth_transactions(), key=lambda tx: (parse_tx_date(tx), str(tx.get("merchant", ""))))
    if not transactions:
        return
    ...
    with st.container(border=True):
        ...
        st.caption(
            "This demo plan uses one deterministic May 2026 transaction history. "
            "Weekly pace comes from Savings transfer transactions plus spending reductions versus Week 1 normal spending."
        )
        st.markdown(
            f"**Goal:** {plan['goal_name']} · **Amount:** {money(plan['goal_cost'])} · "
            f"**Target:** {plan['target_date'].isoformat()} · **Starting saved:** {money(plan.get('progress', 0.0))}"
        )
        st.markdown(
            "**Protected:** Rent / Bills, Groceries. "
            "**Flexible:** Restaurants / dining, Shopping, Entertainment, Coffee. "
            "**Semi-flexible:** Subscriptions. "
            "Byable does not recommend cuts to Rent / Bills or Groceries."
        )
        ...
        for tx in transactions:
            ...
            tx_rows.append(
                {
                    "Week": controlled_demo_week_label(tx),
                    "Date": str(tx.get("date", "")),
                    "Merchant": str(tx.get("merchant", "")),
                    "Category": str(tx.get("category", "")),
                    "Amount": signed_money(amount),
                }
            )
        st.markdown("**Transaction history by week**")
        st.dataframe(pd.DataFrame(tx_rows), width="stretch", hide_index=True)

        totals = {}
        for tx in transactions:
            amount = float(tx.get("amount", 0.0) or 0.0)
            key = (controlled_demo_week_label(tx), str(tx.get("category") or "Other"))
            totals[key] = totals.get(key, 0.0) + amount
        total_rows = [
            {"Week": week, "Category": category, "Total": signed_money(total)}
            for (week, category), total in sorted(totals.items(), key=lambda item: (item[0][0], item[0][1]))
        ]
        st.markdown("**Category totals by week**")
...
def render_goal_plan(plan):
    ...
    with st.container(border=True):
        ...
        st.markdown(f"**Goal:** {plan['goal_name']}")
        st.markdown(f"**Target date:** {target_label}")
        st.progress(progress_ratio)
        projection_label = projected_date_label(behavior_projected_date or projected_date)
        pace_label = f"{money(max(0.0, saved_toward_goal))}/week"
        pace_gap_label = (
            f"{money(behavior_pace_delta)}/week ahead"
            if behavior_pace_delta >= 0
            else f"{money(abs(behavior_pace_delta))}/week needed"
        )
        st.markdown(f"**Status:** {behavior_status}")
        st.markdown(f"**Current projection:** {projection_label}")
        st.markdown(f"**Time gained/lost:** {time_signal}")
        st.markdown(f"**Weekly pace:** {pace_label}")
        st.markdown(f"**Pace gap:** {pace_gap_label}")
        st.caption(
    ...
    render_transaction_intelligence(plan)
    render_affordability_source_of_truth(plan)
    ...
    with insight_col:
        with st.container(border=True):
            ...
            st.caption("Rules-based signals from timeline math and transaction history.")
        ...
        with st.container(border=True):
            ...
            if top_recommendations:
                for row in top_recommendations:
                    ...
                    st.caption(row.get("Recommendation", row.get("Behavior change", "")))
                    st.caption(f"Weekly lift: +{money(row['Recommended weekly cut'])}/week")
            else:
...
st.sidebar.divider()
if st.sidebar.button("Load controlled Hawaii demo history", width="stretch"):
    try:
        result = api_load_realistic_transaction_history()
        st.session_state.pop("goal_plan", None)
        st.sidebar.success(f"Loaded {int(result.get('loaded', 0))} controlled demo transactions.")
        st.rerun()
    except Exception as e:
...
if page == "Trip Planner":
    ...
    if not show_legacy_tools:
        st.stop()
    st.divider()
    st.caption("Advanced legacy affordability engine")
    ...
    if not st.session_state.get("local_budget") or not st.session_state.get("local_transactions"):
        st.info(
            ...
            "You can also load the controlled Hawaii demo history from the sidebar to see how the product works."
        )
    ...
    st.session_state.setdefault("goal_input_date", datetime.date(2026, 8, 18))
